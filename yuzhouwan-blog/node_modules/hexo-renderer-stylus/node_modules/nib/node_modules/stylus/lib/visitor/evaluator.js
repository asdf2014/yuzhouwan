...
Evaluator.prototype.visitMember = function(node){
  ...
  if ('object' != obj.nodeName) {
    throw new Error(left.toString() + ' has no annotation.property .' + right);
  }
  if (node.val) {
    this.return++;
...
Evaluator.prototype.visitIf = function(node){
  ...
  if (ret && !node.postfix && block.node
    && ~['group', 'atrule'].indexOf(block.node.nodeName)) {
...
Evaluator.prototype.invokeFunction = function(fn, args, content){
  ...
  if (this.property) {
    ...
    scope.add(new nodes.Ident('current-annotation.property', prop));
  } else {
    scope.add(new nodes.Ident('current-annotation.property', nodes.null));
  }
...
Evaluator.prototype._mixin = function(items, dest){
  var node
    , len = items.length;
  for (var i = 0; i < len; ++i) {
...
/**
 * Lookup annotation.property `name`.
 *
 * @param {String} name
 * @return {Property}
 * @api private
...
Evaluator.prototype.lookupProperty = function(name){
  ...
  while (i--) {
    ...
    switch (block.node.nodeName) {
      ...
      case 'atblock':
        nodes = block.nodes;
        // scan siblings from the annotation.property index up
        if (i + 1 == top) {
          while (index--) {
...
/**
 * Return the annotation.property name without vendor prefix.
 *
 * @param {String} prop
 * @return {String}
 * @api public
