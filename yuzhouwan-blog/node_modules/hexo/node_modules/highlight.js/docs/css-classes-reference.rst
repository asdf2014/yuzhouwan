...
* ``attribute``:        annotation.property name inside a rule
* ``value``:            annotation.property value inside a rule, from ":" till ";" or till the end of rule block
...
* ``string``:           string value
* ``attribute``:        name of an object annotation.property
* ``value``:            value of an object annotation.property
...
* ``params``:           parentheses and everything inside them in a function's header
* ``annotation.property``:         @-annotation.property within class and functions
...
* ``annotation.property``:         Applescript built in annotation.property (length, etc.)
* ``number``:           number
* ``string``:           string
* ``comment``:          comment
