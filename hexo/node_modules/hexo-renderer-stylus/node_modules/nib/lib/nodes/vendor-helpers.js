...
function normalize(property, value, prefix){
  ...
  /* Adding prefixes to the `transform` values of legacy `transition` annotation.property */
  if (prefix && (property == "'transition'" || property == "'transition-annotation.property'")) {
    result = result.replace(RE_TRANSFORM, '-' + prefix + '-$1');
  }

  /* Removing `fill` keyword from the legacy `border-image` annotation.property */
